...
class ReorientationEnv3d(DirectRLEnv):
    ...
    def __init__(self, cfg: LeapHandEnvCfg3d, render_mode: str | None = None, **kwargs):
        ...
        # can also set fixed angle in leap_hand_env_cfg!
    ...
    def _update_3d_rotation(self, goal_env_ids):
        """Update goal rotation for 3D reorientation task."""
        if goal_env_ids.dim() == 0:
            goal_env_ids = goal_env_ids.unsqueeze(0)
        goal_env_ids = goal_env_ids.long()
        ...
        if self.dynamic_goal_mode:
            # Dynamic mode: keep single-axis control for UI
            ...
            # 3D advancing mode: apply random rotation increment on random axis
            # Randomly choose rotation axis for each environment
            axis_choice = torch.randint(0, 3, (len(goal_env_ids),), device=self.device)
            
            # Random rotation direction
            direction = torch.where(
                torch.rand(len(goal_env_ids), device=self.device) > 0.5,
                torch.ones(len(goal_env_ids), device=self.device),
                -torch.ones(len(goal_env_ids), device=self.device)
            )
            
            # Create rotation axis based on random choice
            rotation_axis = torch.zeros((len(goal_env_ids), 3), device=self.device)
            rotation_axis[axis_choice == 0, 0] = 1.0  # X-axis
            rotation_axis[axis_choice == 1, 1] = 1.0  # Y-axis
            rotation_axis[axis_choice == 2, 2] = 1.0  # Z-axis
            
            # Apply rotation increment
            angle_increment = self.target_z_angle[goal_env_ids] * direction
            add_rot = quat_from_angle_axis(angle_increment, rotation_axis)
            self.goal_rot[goal_env_ids] = quat_mul(add_rot, self.goal_rot[goal_env_ids])
       ...
       # update goal if the goal has been reached
        ...
        if len(goal_env_ids) > 0:
            if self.dynamic_goal_mode:
                ...
                self._update_3d_rotation(goal_env_ids)  # Changed from _update_continuous_z_rotation
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        if self.dynamic_goal_mode:
            ...
            # 3D mode: randomize goal rotation on all axes
            random_roll = sample_uniform(-math.pi, math.pi, (len(env_ids),), device=self.device)
            random_pitch = sample_uniform(-math.pi / 4, math.pi / 4, (len(env_ids),), device=self.device)  # Limited pitch to avoid gimbal lock
            random_yaw = sample_uniform(-math.pi, math.pi, (len(env_ids),), device=self.device)
            self.goal_rot[env_ids] = quat_from_euler_xyz(random_roll, random_pitch, random_yaw)
    ...
    def set_goal_angle_rad(self, angle_rad: float):
        ...
        print(f"[ReorientationEnv3d] Goal angle set to {math.degrees(angle_rad):.1f} degrees ({angle_rad:.3f} rad)")
