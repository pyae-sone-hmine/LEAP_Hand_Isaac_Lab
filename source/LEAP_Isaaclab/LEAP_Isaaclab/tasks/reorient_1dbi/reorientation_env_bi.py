...
import time
if TYPE_CHECKING:
    from LEAP_Isaaclab.tasks.leap_hand_reorient.leap_hand_env_cfg import LeapHandEnvCfgBi

from LEAP_Isaaclab.utils import adr_utils, obs_utils
from LEAP_Isaaclab.utils.adr import LeapHandADR

class ReorientationEnvBi(DirectRLEnv):
    cfg: LeapHandEnvCfgBi

    def __init__(self, cfg: LeapHandEnvCfgBi, render_mode: str | None = None, **kwargs):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        if self.cfg.enable_adr:
            # still use successes as a coarse ADR signal; stepping is gone so this measures
            # success-per-episode-time rather than rotations-per-second
            adr_criteria = (
                (self.consecutive_successes / max(self.cfg.z_rotation_steps, 1))
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        if self.cfg.enable_adr:
            adr_criteria = (
                (self.consecutive_successes.float().mean() / max(self.cfg.z_rotation_steps, 1))
                / (self.randomized_episode_lengths.float().mean() * self.cfg.sim.dt * self.cfg.decimation)
            ).float().mean()
        ...
        # initialize goal rotation: sample a single desired z-angle per env for this episode
        self._compute_intermediate_values()
        ...
        self.current_goal_angle_rad = 0.0  # scalar convenience; per-env goals are stored in goal_rot
    ...
    def _update_goals(self, new_success: torch.Tensor):
        ...
        - After hold phase, sample new goal and continue episode
        """
    ...
    def set_goal_angle_rad(self, angle_rad: float):
        ...
        goal_pos = self.goal_pos + self.scene.env_origins
        self.goal_markers.visualize(goal_pos, self.goal_rot)
        # No freezing/stepping: policy continues to control the hand freely.
...
def compute_rewards(
    ...
    # regularization + joint posture penalty
    ...
    #   - regularize actions and posture and torques
